...
def page1():
    ...
    session_state.ng = st.number_input('Number of Eletricity Generators',value = session_state.ng)
    # Tdig = st.number_input('Temperature of the digestor (C°): ',value = 37)
    session_state.debt_level = st.number_input('Debt level of the project',value = session_state.debt_level)
    session_state.V_cng_p = st.number_input('Volume of Gas upgraded to biodiesel for manure transportation: ',value = session_state.V_cng_p)
    session_state.farm1 = st.number_input('is farm 1 active: ',value = session_state.farm1)
    session_state.farm2 = st.number_input('is farm 2 active: ',value = session_state.farm2)
    session_state.farm3 = st.number_input('is farm 3 active: ',value = session_state.farm3)
    session_state.farm4 = st.number_input('is farm 4 active: ',value = session_state.farm4)
    session_state.farm5 = st.number_input('is farm 5 active: ',value = session_state.farm5)
    session_state.farm6 = st.number_input('is farm 6 active: ',value = session_state.farm6)
    session_state.farm7 = st.number_input('is farm 7 active: ',value = session_state.farm7)
    session_state.lam = st.number_input('Multi-objective (1 full NPV, 0 full emissions): ',value = session_state.lam)
...
def page2():
    ...
    session_state.e_priceS = st.number_input('Price of electrical energy sold (R$/kWh): ',value = session_state.e_priceS)
    session_state.e_c = st.number_input('Electrical energy consumed in the farms (kWh/year): ',value = session_state.e_c)
    session_state.e_priceB = st.number_input('Price of electrical energy bought (R$/kWh): ',value = session_state.e_priceB)
    st.write('Fertilizer: ')
    session_state.f_used = st.number_input('Quantity of fertilizer used in the farms (kg/year): ',value = session_state.f_used)
    session_state.p_bf = st.number_input('Price of buying fertilizer used in the farms (R$/kg): ',value = session_state.p_bf)
    session_state.p_l = st.number_input('Selling price of fertilizer surplus (R$/kg): ',value = session_state.p_l)
    st.write('Transportation: ')
    session_state.c_rskm = st.number_input('Cost to transport manure (R$/km): ',value = session_state.c_rskm )
    session_state.c_km = session_state.c_rskm
    session_state.T_m3_km_cng = st.number_input('Truck average CNG consumption (m^3/km): ',value = session_state.T_m3_km_cng)
    session_state.T_L_km_diesel = st.number_input('Truck average diesel consumption (L/km): ',value = session_state.T_L_km_diesel)
    session_state.V_per_truck = st.number_input('Truck maximum capacity (m^3): ',value = session_state.V_per_truck)
    st.write('Biogas: ')
    session_state.C_V_gas = st.number_input('Cost to produce biogas (R$/m^3): ',value = session_state.C_V_gas)
    session_state.p_g = st.number_input('Selling price of biogas (R$/m^3): ',value = session_state.p_g)
    session_state.C_upgrade_cng = st.number_input('Cost to upgrade biogas to CNG (R$/m^3): ',value = session_state.C_upgrade_cng)
    st.write('Generator: ')
    session_state.g_d = st.number_input('Cost purchase eletricity generator (R$/unit): ',value = session_state.g_d)
    session_state.g_eff = st.number_input('Eletricity generator efficiency with biogas (%): ',value = session_state.g_eff)
    session_state.g_power = st.number_input('Eletricity generator power capacity (kW): ',value = session_state.g_power)
    session_state.g_m = st.number_input('Yealy cost of maintenance of eletricity generator (R$/(unit*year)): ',value = session_state.g_m)
    st.write('Biodigestor: ')
    session_state.i_main_cost = st.number_input('Yealy cost of maintenance of biodigestor (% of investment/year): ',value = session_state.i_main_cost)
    session_state.L = st.number_input('Biodigestor life (years): ',value = session_state.L)
    st.write('Financial: ')
    session_state.kd = st.number_input('Cost of debt (%): ',value = session_state.kd)
    session_state.max_debt = st.number_input('Maximum debt allowed (%): ',value = session_state.max_debt)
    session_state.ke = st.number_input('Cost of equity (%): ',value = session_state.ke)
    session_state.tax = st.number_input('Tax on profits (%): ',value = session_state.tax)
    session_state.USS_to_RS = st.number_input('Corversion of US$ to R$ (R$/US$): ',value = session_state.USS_to_RS)
    session_state.working_days = st.number_input('Working days per year (days/year): ',value = session_state.working_days)
    session_state.working_hours = st.number_input('Working hours per day (hours/day): ',value = session_state.working_hours)
main()
# print(dict_totalUser['e_priceS'])
